...
namespace DecryptCocos2dAsset
{
    class Program
    {
        ...
        {
            SetKey(0x72c159a2,0x4b3f9693,0x97bc2991,0x8a8ef15b);
//            SetKey(0xF68C6273, 0x07C32116, 0x4AF4F1AC, 0xBF0988A6);
            InitializeKey();
            //ExportKey();

            var bytes = File.ReadAllBytes(args[0] + ".pvr.ccz");
//            var bytes = File.ReadAllBytes("role_2_6.pvr.ccz");
            // skip header(12byte)
            ...
            File.WriteAllBytes(args[0] + ".pvr", bytes);
        }
    }
}
